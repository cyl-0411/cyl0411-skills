...
### Python Runtime (resolve once)

Before running any bundled Python script, resolve one interpreter and refer to it as
`<PPT_PYTHON>` for the rest of the workflow:

1. In Codex Desktop, call the workspace dependency loader and prefer its bundled
   Python executable.
2. Otherwise, choose a Python or virtual-environment interpreter that
   can run `import pptx, lxml, PIL` successfully.
3. Run `"<PPT_PYTHON>" -X utf8 -c "import pptx, lxml, PIL"` as the preflight check. If it
   fails, install `requirements.txt` into that interpreter or select another one.

Do not rely on a platform-specific Python command alias. Always add `-X utf8` so
Unicode diagnostics are safe in Windows terminals. Every downstream command must
use `"<PPT_PYTHON>" -X utf8`.
...
|---------------|---------|
| PDF file | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/source_to_md/pdf_to_md.py <file>` |
| DOCX / Word / Office document | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/source_to_md/doc_to_md.py <file>` |
| XLSX / XLSM / Excel workbook | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/source_to_md/excel_to_md.py <file>` |
| CSV / TSV | Read directly as plain-text table source |
| PPTX / PowerPoint deck | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/source_to_md/ppt_to_md.py <file>` |
| EPUB / HTML / LaTeX / RST / other | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/source_to_md/doc_to_md.py <file>` |
| Web link | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/source_to_md/web_to_md.py <URL>` |
| WeChat / high-security site | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/source_to_md/web_to_md.py <URL>` (requires `curl_cffi`, included in `requirements.txt`) |
...
```bash
"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/project_manager.py init <project_name> --format <format>
```
...
| Situation | Action |
|-----------|--------|
| Has source files (PDF/MD/etc.) | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/project_manager.py import-sources <project_path> <source_files...> --move` |
...
3. Run:
   ```bash
   "<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/latex_render.py <project_path>
   ```
...
```bash
"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/analyze_images.py <project_path>/images
```
...
|---|---|---|
| `ai` | `references/image-generator.md` | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/image_gen.py --manifest <project_path>/images/image_prompts.json` |
| `web` | `references/image-searcher.md` | `"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/image_search.py ...` |
...
```bash
"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --live
```
...
```bash
"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/svg_quality_checker.py <project_path>
```
...
```bash
"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/total_md_split.py <project_path>
```

**Step 7.2** — SVG post-processing (icon embedding / image crop & embed / text flattening / rounded rect to path):
```bash
"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/finalize_svg.py <project_path>
```

**Step 7.3** — Export PPTX (embeds speaker notes by default):
```bash
"<PPT_PYTHON>" -X utf8 ${SKILL_DIR}/scripts/svg_to_pptx.py <project_path>
...
- Local preview: `"<PPT_PYTHON>" -X utf8 -m http.server -d <project_path>/svg_final 8000`
